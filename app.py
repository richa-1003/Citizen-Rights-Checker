# # app.py — Citizen Rights Checker
# # Redesigned UI: warm parchment tone, legal/judicial aesthetic
...
#     background: #F7F3ED;
#     background-image:
#         radial-gradient(ellipse at 20% 10%, rgba(180,155,110,0.12) 0%, transparent 55%),
#         radial-gradient(ellipse at 80% 90%, rgba(120,100,70,0.08) 0%, transparent 55%);
#     min-height: 100vh;
# }
...
#     box-shadow: 0 0 0 6px rgba(44,36,22,0.1), 0 0 0 12px rgba(44,36,22,0.05);
# }
# .hero-title {
#     font-family: 'Lora', serif;
#     font-size: 2.6rem;
...
# /* Example chips */
# div[data-testid*="ex_"] .stButton > button,
# div[key*="ex_"] .stButton > button {
#     background: rgba(44,36,22,0.06) !important;
#     color: #5C4A32 !important;
#     border: 1px solid rgba(44,36,22,0.14) !important;
...
#     padding: 10px 14px;
# }
# .conf-label { font-size: 12px; color: #8B6B3D; white-space: nowrap; }
# .conf-track { flex: 1; height: 3px; background: rgba(44,36,22,0.1); border-radius: 2px; }
# .conf-fill { height: 3px; border-radius: 2px; background: #8B6B3D; }
# .conf-pct { font-size: 12px; font-weight: 500; color: #5C4A32; min-width: 34px; text-align: right; }
...
#             <path d="M16 4L6 9v7c0 6.2 4.3 12 10 13.4C21.7 28 26 22.2 26 16V9L16 4z"
#                   fill="none" stroke="#C8A96E" stroke-width="1.5"
#                   stroke-linejoin="round"/>
#             <path d="M11 16l3.5 3.5L21 13" stroke="#C8A96E"
...
#     placeholder="e.g. My employer refuses to pay me for overtime work and threatens to fire me if I complain...",
#     height=100,
...
# st.markdown('<p class="examples-label">Or try an example</p>', unsafe_allow_html=True)
...
#     st.markdown(f"""
#     <div class="conf-wrap">
#         <span class="conf-label">Relevance confidence</span>
...
#     if rights:
#         st.markdown('<p class="section-label">Constitutional provisions that may apply</p>',
#                     unsafe_allow_html=True)
#         for r in rights:
#             st.markdown(f"""
...
#         information grounded in the text of the Constitution of India. It is
#         <strong>not legal advice</strong>. Rights under Part III generally apply
#         against the State, not private individuals — disputes with private parties
#         may be better addressed through civil or tenancy law. For your specific
...
#             <path d="M24 44s16-7 16-20V11L24 5 8 11v13c0 13 16 20 16 20z"/>
#             <path d="M17 24l5.5 5.5L31 20"/>
#         </svg>
...
# app.py — Citizen Rights Checker
# Warm parchment aesthetic, falling legal symbols background
...
#     y: randomY ? rand(-canvas.height, canvas.height) : rand(-80,-10),
#     size: rand(12,28),
#     speed: rand(0.5, 1.2),
...
# function drawScale(x,y,s,op){
#   ctx.save();
#   ctx.strokeStyle='rgba(139,107,61,'+op+')';
...
#   ctx.beginPath(); ctx.moveTo(x-s,y); ctx.lineTo(x-s*0.7,y+s*0.5); ctx.lineTo(x-s*1.3,y+s*0.5); ctx.closePath(); ctx.stroke();
#   ctx.beginPath(); ctx.moveTo(x+s,y); ctx.lineTo(x+s*0.7,y+s*0.5); ctx.lineTo(x+s*1.3,y+s*0.5); ctx.closePath(); ctx.stroke();
#   ctx.restore();
# }

# function drawLeaf(x,y,s,op){
#   ctx.save();
#   ctx.fillStyle='rgba(139,107,61,'+op+')';
#   ctx.beginPath();
#   ctx.moveTo(x, y-s);
#   ctx.bezierCurveTo(x+s,y-s*0.5, x+s,y+s*0.5, x,y+s);
#   ctx.bezierCurveTo(x-s,y+s*0.5, x-s,y-s*0.5, x,y-s);
#   ctx.fill();
#   ctx.strokeStyle='rgba(139,107,61,'+(op*0.7)+')';
#   ctx.lineWidth=s*0.06;
#   ctx.beginPath(); ctx.moveTo(x,y-s); ctx.lineTo(x,y+s*1.2); ctx.stroke();
#   ctx.restore();
# }

# function drawRibbon(x,y,s,op){
#   ctx.save();
#   ctx.fillStyle='rgba(44,36,22,'+op+')';
#   ctx.fillRect(x-s,y-s*0.12,s*2,s*0.24);
#   ctx.beginPath(); ctx.arc(x,y,s*0.15,0,Math.PI*2); ctx.fill();
#   ctx.restore();
# }

# function drawSection(x,y,s,op){
#   ctx.save();
#   ctx.fillStyle='rgba(139,107,61,'+op+')';
...
#     ctx.rotate(p.rot);
#     if(p.type===0) drawScale(0,0,p.size,p.op);
#     else if(p.type===1) drawLeaf(0,0,p.size*0.7,p.op);
#     else if(p.type===2) drawRibbon(0,0,p.size*1.2,p.op);
#     else drawSection(0,0,p.size*1.4,p.op);
#     ctx.restore();
#     p.y+=p.speed; p.x+=p.drift; p.rot+=p.rotS;
#     if(p.y>canvas.height+80) particles[i]=newParticle(false);
#   }
